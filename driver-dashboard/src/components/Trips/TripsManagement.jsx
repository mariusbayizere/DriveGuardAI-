...
const TripsManagement = () => {
  ...
  const [formData,          setFormData]          = useState({
    tripName: '', startTime: '', endTime: '', status: 'ONGOING', driverId: '', vehicleId: ''
  });
  ...
  const fetchRelatedData = useCallback(async () => {
    try {
      const [driversRes, vehiclesRes] = await Promise.all([
        tripService.getAllDrivers(),
        tripService.getAllVehicles()
      ]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const tripData = {
        ...
        vehicle:   { vehicleId: parseInt(formData.vehicleId) }
      };
      if (editingTrip) {
        await tripService.updateTrip(editingTrip.tripId, tripData);
  ...
  const handleEdit = (trip) => {
    ...
    setFormData({
      ...
      startTime: fmt(trip.startTime),
      endTime:   fmt(trip.endTime),
  ...
  const resetForm = () => {
    setFormData({ tripName: '', startTime: '', endTime: '', status: 'ONGOING', driverId: '', vehicleId: '' });
    setFormErrors({});
    setEditingTrip(null);
  };
  ...
  let filtered = trips.filter(trip => {
    ...
    const matchesSearch =
      ...
      (trip.driver?.user?.firstName || '').toLowerCase().includes(q) ||
      (trip.driver?.user?.lastName  || '').toLowerCase().includes(q) ||
  ...
  filtered = [...filtered].sort((a, b) => {
    let av, bv;
    if      (sortBy === 'id')    { av = a.tripId;                          bv = b.tripId; }
    else if (sortBy === 'name')  { av = (a.tripName||'').toLowerCase();    bv = (b.tripName||'').toLowerCase(); }
    else if (sortBy === 'start') { av = new Date(a.startTime||0).getTime(); bv = new Date(b.startTime||0).getTime(); }
    else if (sortBy === 'end')   { av = new Date(a.endTime||0).getTime();   bv = new Date(b.endTime||0).getTime(); }
  ...
  const getStatusStyle = (status) => STATUS_CLASSES[status] || STATUS_CLASSES.ONGOING;
    ...
    <>
      <style>{`
        ...
        * { font-family: 'Inter', sans-serif; box-sizing: border-box; }
        ...
        .trip-card { overflow: visible !important; }
      `}</style>
      ...
      <div className="px-4 sm:px-6 pt-7 pb-12 w-full">
        ...
        <div className="flex flex-col sm:flex-row flex-wrap gap-3 mb-6 items-stretch sm:items-center"
          ...
          <div className="flex items-center flex-1 min-w-0 sm:min-w-[260px]
            ...
            <input
              type="text"
              placeholder="Search by trip name, driver, plate..."
              value={searchTerm}
          ...
          <div className="flex gap-2 items-center">
            <button className="icon-circle" title="Refresh" onClick={fetchTrips}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        ...
        {loading && !showModal && (
          <div className="bg-white rounded-2xl shadow-[0_10px_30px_rgba(0,0,0,0.08)] border border-gray-200 py-4 text-center">
            ...
            <p className="text-gray-400 pb-6">Loading trips…</p>
          </div>
        )}
          ...
          <>
            ...
            <div className="hidden md:block bg-white rounded-2xl shadow-[0_10px_30px_rgba(0,0,0,0.08)]
                ...
                <table className="w-full border-collapse">
                  <thead>
                    <tr>
                      {[
                        { label:'ID',         col:'id',    w:'6%'  },
                        { label:'TRIP NAME',  col:'name',  w:'22%' },
                        { label:'START TIME', col:'start', w:'18%' },
                        { label:'END TIME',   col:'end',   w:'18%' },
                        { label:'DRIVER',     col:null,    w:'18%' },
                        { label:'STATUS',     col:null,    w:'10%' },
              ...
              <table className="w-full border-collapse">
                <tbody>
                  {paginated.length === 0 ? (
                    ...
                    const sc = getStatusStyle(trip.status);
                    return (
                      <tr key={trip.tripId} className="trow">
                        <td className="px-5 py-4 w-[6%] font-semibold text-[#84CC16] text-[13px]">{trip.tripId}</td>
                        <td className="px-5 py-4 w-[22%] text-[13px]">
                          <div className="font-bold text-gray-800">{trip.tripName || '—'}</div>
                        </td>
                        <td className="px-5 py-4 w-[18%] text-[13px]">
                          <div className="font-semibold text-gray-800">{formatDateTime(trip.startTime)}</div>
                        </td>
                        <td className="px-5 py-4 w-[18%] text-[13px]">
                          <div className="font-semibold text-gray-800">{formatDateTime(trip.endTime)}</div>
                        </td>
                        ...
                        <td className="px-5 py-4 w-[10%] text-[13px]">
                        ...
                        <td className="px-5 py-4 w-[8%]">
                          <div className="flex gap-2">
                            <button onClick={() => handleEdit(trip)} title="Edit"
                            ...
                            <button onClick={() => handleDeleteClick(trip.tripId)} title="Delete"
              ...
              {paginated.length > 0 && (
                <div className="bg-[rgba(132,204,22,0.05)] px-5 py-4 border-t border-gray-200
                  ...
                  <span>
                    Showing <strong>{startIndex + 1}</strong>–<strong>{Math.min(startIndex + itemsPerPage, filtered.length)}</strong> of <strong>{filtered.length}</strong> trips
                  </span>
            ...
            <div className="md:hidden mt-4">
              <p className="text-sm text-gray-500 mb-3 px-1">
                Showing <strong>{startIndex + 1}–{Math.min(startIndex + itemsPerPage, filtered.length)}</strong> of <strong>{filtered.length}</strong> trips
              </p>
              ...
              {paginated.length === 0 ? (
                ...
                <div className="flex flex-col gap-3">
                  {paginated.map(trip => {
                    const isOpen = expandedCardId === trip.tripId;
                    const sc = getStatusStyle(trip.status);
                    return (
                      <div key={trip.tripId}
                        className="trip-card bg-white rounded-2xl border border-gray-200
                        ...
                        <div className="flex items-center justify-between px-4 pt-4 pb-3">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <div className="text-gray-400 text-xs mt-0.5">ID: {trip.tripId}</div>
                            </div>
                          </div>
                          <button
                            onClick={() => toggleCard(trip.tripId)}
                        ...
                        <div className="px-4 pb-3 flex items-center gap-2 text-sm text-gray-500">
                          ...
                          <span className="font-medium text-gray-700 text-xs ml-1">{formatDateTime(trip.startTime)}</span>
                        </div>
                            ...
                            <div className="px-4 pb-3 flex items-center gap-2 text-sm text-gray-500">
                              ...
                              <span className="font-medium text-gray-700 text-xs ml-1">
			 ...
			 {/* ── Status badge ── */}
                        ...
                        {isOpen && (
                          <div className="anim-expandIn border-t border-gray-100 px-4 py-4">
                            ...
                            {/* Edit + Delete */}
                            <div className="flex gap-2">
                              <button onClick={() => handleEdit(trip)} className="mob-edit-btn">
                              ...
                              <button onClick={() => handleDeleteClick(trip.tripId)} className="mob-del-btn">
                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                                  <polyline points="3 6 5 6 21 6"/>
                                  <path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/>
                                  <line x1="10" y1="11" x2="10" y2="17"/>
        ...
        <div className="anim-fadeIn fixed inset-0 bg-black/50 flex items-center justify-center z-[1000] p-4"
          ...
          <div className="anim-slideUp bg-white rounded-2xl w-full max-w-[600px] max-h-[90vh] overflow-y-auto
            ...
            <div className="bg-gradient-to-r from-[#84CC16] to-[#65A30D] text-white px-6 py-5
              ...
              <h2 className="m-0 text-xl font-bold">{editingTrip ? 'Edit Trip' : 'Add New Trip'}</h2>
            ...
            <div className="p-5 sm:p-6">
              ...
              {/* Trip Name */}
              <div className="mb-5">
                <label className="block font-semibold mb-2 text-gray-800 text-sm">Trip Name *</label>
                <input type="text" name="tripName" value={formData.tripName}
                  onChange={handleInputChange}
                  placeholder="e.g. Morning Delivery - Kigali to Muhanga"
                  className={`${INPUT_BASE} ${formErrors.tripName ? INPUT_ERR : ''}`}/>
                {formErrors.tripName && <p className="text-red-400 text-xs mt-1">⚠ {formErrors.tripName}</p>}
              </div>

              {/* Start + End Time */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
                <div>
                  <label className="block font-semibold mb-2 text-gray-800 text-sm">Start Time *</label>
                  <input type="datetime-local" name="startTime" value={formData.startTime}
                    onChange={handleInputChange}
                    min={!editingTrip ? getNowLocal() : undefined}
                    className={`${INPUT_BASE} ${formErrors.startTime ? INPUT_ERR : ''}`}/>
                  {formErrors.startTime && <p className="text-red-400 text-xs mt-1">⚠ {formErrors.startTime}</p>}
                </div>
                <div>
                  <label className="block font-semibold mb-2 text-gray-800 text-sm">End Time *</label>
                  <input type="datetime-local" name="endTime" value={formData.endTime}
                    onChange={handleInputChange}
                    min={!editingTrip ? (formData.startTime || getNowLocal()) : undefined}
                    className={`${INPUT_BASE} ${formErrors.endTime ? INPUT_ERR : ''}`}/>
                  {formErrors.endTime && <p className="text-red-400 text-xs mt-1">⚠ {formErrors.endTime}</p>}
                </div>
              </div>
              ...
              <div className="mb-5">
                ...
                {vehicles && vehicles.length > 0 ? (
                  <select name="vehicleId" value={formData.vehicleId} onChange={handleInputChange}
                    ...
                    {vehicles.filter(v => v && v.vehicleId).map((vehicle, i) => (
                      <option key={`vehicle-${vehicle.vehicleId}-${i}`} value={vehicle.vehicleId}>
                        {vehicle.licensePlate || 'N/A'} - {vehicle.make || ''} {vehicle.model || ''}
                      </option>
                    ))}
                  </select>
                ) : (
      ...
      {showConfirmDialog && (
        <div className="anim-fadeIn fixed inset-0 bg-black/60 flex items-center justify-center z-[2000] p-4"
          ...
          <div className="anim-slideUp bg-white rounded-2xl w-full max-w-[420px] p-8
            ...
            <div className="text-lg font-bold text-gray-800 text-center mb-2">Delete Trip?</div>
            <div className="text-sm text-gray-500 text-center mb-6 leading-relaxed">
              Are you sure you want to delete this trip? This action cannot be undone and all associated data will be permanently removed.
            </div>
