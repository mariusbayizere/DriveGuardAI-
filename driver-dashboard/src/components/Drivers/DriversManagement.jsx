...
const DriversManagement = () => {
  ...
  const [formData,          setFormData]          = useState({
    licenseNumber: '', hireDate: '', safetyScore: '', status: 'ACTIVE', userId: ''
  });
  const [formErrors, setFormErrors] = useState({});
  const [users,      setUsers]      = useState([]);

  // ── FIX 3: useCallback so these functions are stable references ──
  const fetchDrivers = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
      const r = await driverService.getAllDrivers();
      setDrivers(r.data);
      setCurrentPage(1);
    } catch (err) {
      setError('Failed to fetch drivers: ' + (err.response?.data?.message || err.message));
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const fetchUsers = useCallback(async () => {
    try {
      const r = await driverService.getAllUsers();
      setUsers(r.data);
    } catch (err) {
  ...
  // ── FIX 3: runs only once on mount, no infinite loop ──
  useEffect(() => {
    fetchDrivers();
    fetchUsers();
  ...
  const validateForm = () => {
    ...
    if (!formData.safetyScore)          errors.safetyScore   = 'Safety score is required';
    else if (formData.safetyScore < 0 || formData.safetyScore > 100)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingDriver) {
        await driverService.updateDriver(editingDriver.id, payload);
        setSuccess(`Driver "${formData.licenseNumber}" updated successfully!`);
      } else {
        await driverService.createDriver(payload);
        setSuccess(`Driver "${formData.licenseNumber}" created successfully!`);
      }
      fetchDrivers();
      resetForm();
      setShowModal(false);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      setSuccess(`Driver "${driver?.licenseNumber}" deleted successfully!`);
      fetchDrivers();
    } catch (err) {
  ...
  const resetForm = () => {
    setFormData({ licenseNumber: '', hireDate: '', safetyScore: '', status: 'ACTIVE', userId: '' });
    setFormErrors({});
  ...
  filtered = [...filtered].sort((a, b) => {
    ...
    else if (sortBy === 'license')     { av = a.licenseNumber.toLowerCase(); bv = b.licenseNumber.toLowerCase(); }
    else if (sortBy === 'safetyScore') { av = a.safetyScore;                 bv = b.safetyScore; }
    ...
    <>
      <style>{`
        ...
        * { font-family: 'Inter', sans-serif; box-sizing: border-box; }
        ...
        /* FIX 1: cards must NOT clip children — the native <select> dropdown
           needs to escape the card boundary on mobile */
        .driver-card { overflow: visible !important; }
      `}</style>
      ...
      <div className="px-4 sm:px-6 pt-7 pb-12 w-full">
        ...
        {/* ── Search + Filters ──
            FIX 1: position:relative + z-index so the select dropdown
            is not hidden behind sibling elements */}
        <div className="flex flex-col sm:flex-row flex-wrap gap-3 mb-6 items-stretch sm:items-center"
          ...
          {/* Status filter — native <select> renders its dropdown outside
              the DOM paint order so overflow:hidden on parents won't clip it */}
          <select
            value={filterStatus}
          ...
          <div className="flex gap-2 items-center">
            <button className="icon-circle" title="Refresh" onClick={fetchDrivers}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        ...
        {loading && !showModal && (
          <div className="bg-white rounded-2xl shadow-[0_10px_30px_rgba(0,0,0,0.08)] border border-gray-200 py-4 text-center">
            ...
            <p className="text-gray-400 pb-6">Loading drivers…</p>
          </div>
        )}
          ...
          <>
            ...
            <div className="hidden md:block bg-white rounded-2xl shadow-[0_10px_30px_rgba(0,0,0,0.08)]
                ...
                <table className="w-full border-collapse">
                  <thead>
                    <tr>
                      {[
                        { label:'ID',           col:'id',          w:'7%'  },
                        { label:'LICENSE',      col:'license',     w:'22%' },
                        { label:'DRIVER NAME',  col:null,          w:'23%' },
                        { label:'SAFETY SCORE', col:'safetyScore', w:'16%' },
                        { label:'STATUS',       col:null,          w:'16%' },
                        { label:'ACTIONS',      col:null,          w:'16%' },
                      ].map(({ label, col, w }) => (
                ...
                <tbody>
                  {paginated.length === 0 ? (
                    ...
                    <tr key={d.id} className="trow">
                      <td className="px-5 py-4 w-[7%] font-semibold text-[#84CC16] text-[13px]">{d.id}</td>
                      <td className="px-5 py-4 w-[22%] text-[13px]">
                        <div className="font-semibold text-gray-800">{d.licenseNumber}</div>
                        <div className="text-gray-400 text-xs mt-0.5">
                          {d.hireDate ? new Date(d.hireDate).toLocaleDateString() : '—'}
                        </div>
                      </td>
                      <td className="px-5 py-4 w-[23%] text-[13px] text-gray-500">
                      ...
                      <td className="px-5 py-4 w-[16%] text-[13px]">
                        <span className={`inline-block px-3 py-1 rounded-full font-semibold text-[11px]
                                          bg-[rgba(132,204,22,0.1)] ${getSafetyScoreColor(d.safetyScore)}`}>
                          {d.safetyScore}%
                        </span>
                      </td>
                      <td className="px-5 py-4 w-[16%] text-[13px]">
                        <span className={`inline-block px-3 py-1 rounded-full font-semibold text-[11px]
                                          ...
                                          ${STATUS_CLASSES[d.status] || STATUS_CLASSES.INACTIVE}`}>
                          {d.status}
                        </span>
                      </td>
                      <td className="px-5 py-4 w-[16%]">
                        <div className="flex gap-2">
                          ...
                          <button onClick={() => handleDeleteClick(d.id)} title="Delete"
              ...
              {paginated.length > 0 && (
                <div className="bg-[rgba(132,204,22,0.05)] px-5 py-4 border-t border-gray-200
                  ...
                  <span>
                    Showing <strong>{startIndex + 1}</strong>–<strong>{Math.min(startIndex + itemsPerPage, filtered.length)}</strong> of <strong>{filtered.length}</strong> drivers
                  </span>
            ...
            <div className="md:hidden mt-4">
              <p className="text-sm text-gray-500 mb-3 px-1">
                Showing <strong>{startIndex + 1}–{Math.min(startIndex + itemsPerPage, filtered.length)}</strong> of <strong>{filtered.length}</strong> drivers
              </p>
              ...
              {paginated.length === 0 ? (
                ...
                <div className="flex flex-col gap-3">
                  {paginated.map(d => {
                    const isOpen = expandedCardId === d.id;
                    return (
                      <div key={d.id}
                        className="driver-card bg-white rounded-2xl border border-gray-200
                        ...
                        <div className="flex items-center justify-between px-4 pt-4 pb-3">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <div className="text-gray-400 text-xs mt-0.5">ID: {d.id}</div>
                            </div>
                          </div>
                          <button
                        ...
                        <div className="px-4 pb-3 flex items-center gap-2 text-sm text-gray-500">
                          ...
                          <span className="font-medium text-gray-700">{d.licenseNumber}</span>
                        ...
                        {isOpen && (
                          <div className="anim-expandIn border-t border-gray-100 px-4 py-4">
                            ...
                            {/* FIX 2: phoneNumber — correct field name from Users.java */}
                              ...
                              <div className="w-7 h-7 rounded-full bg-purple-50 flex items-center justify-center shrink-0">
                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none"
                                  ...
                                  <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07
                                           ...
                                           2 2 0 0 1 3.6 1h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81
                                           a2 2 0 0 1-.45 2.11L7.91 8.6a16 16 0 0 0 6 6l.91-.91
                                           a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7
                                           A2 2 0 0 1 21.73 16z"/>
                                </svg>
                              </div>
                            ...
                            {/* FIX 2: Users.java has no gender — show userRole instead */}
                            <div className="flex items-center gap-2 mb-4 text-sm text-gray-600">
                              ...
                              <span className="ml-auto text-gray-700 text-sm font-medium capitalize">
                                {d.user?.userRole
                                  ? d.user.userRole.charAt(0) + d.user.userRole.slice(1).toLowerCase()
                                  : '—'}
                              </span>
                            </div>
                            ...
                            {/* Edit + Delete */}
                            <div className="flex gap-2">
                              <button onClick={() => handleEdit(d)} className="mob-edit-btn">
                              ...
                              <button onClick={() => handleDeleteClick(d.id)} className="mob-del-btn">
                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                                  <polyline points="3 6 5 6 21 6"/>
                                  <path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/>
                                  <line x1="10" y1="11" x2="10" y2="17"/>
        ...
        <div className="anim-fadeIn fixed inset-0 bg-black/50 flex items-center justify-center z-[1000] p-4"
          ...
          <div className="anim-slideUp bg-white rounded-2xl w-full max-w-[600px] max-h-[90vh] overflow-y-auto
            ...
            <div className="bg-gradient-to-r from-[#84CC16] to-[#65A30D] text-white px-6 py-5
              ...
              <h2 className="m-0 text-xl font-bold">{editingDriver ? 'Edit Driver' : 'Add New Driver'}</h2>
            ...
            <div className="p-5 sm:p-6">
              <div className="mb-5">
                <label className="block font-semibold mb-2 text-gray-800 text-sm">License Number *</label>
                <input type="text" name="licenseNumber" value={formData.licenseNumber}
                  onChange={handleInputChange} placeholder="Enter license number"
                  className={`${INPUT_BASE} ${formErrors.licenseNumber ? INPUT_ERR : ''}`}/>
                {formErrors.licenseNumber && <p className="text-red-400 text-xs mt-1">{formErrors.licenseNumber}</p>}
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
                <div>
                  <label className="block font-semibold mb-2 text-gray-800 text-sm">Hire Date *</label>
                  <input type="date" name="hireDate" value={formData.hireDate}
                    onChange={handleInputChange}
                    className={`${INPUT_BASE} ${formErrors.hireDate ? INPUT_ERR : ''}`}/>
                  {formErrors.hireDate && <p className="text-red-400 text-xs mt-1">{formErrors.hireDate}</p>}
                </div>
                <div>
                  <label className="block font-semibold mb-2 text-gray-800 text-sm">Safety Score (0-100) *</label>
                  <input type="number" name="safetyScore" value={formData.safetyScore}
                    onChange={handleInputChange} placeholder="Enter safety score" min="0" max="100"
                    className={`${INPUT_BASE} ${formErrors.safetyScore ? INPUT_ERR : ''}`}/>
                  {formErrors.safetyScore && <p className="text-red-400 text-xs mt-1">{formErrors.safetyScore}</p>}
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
                <div>
                ...
                <div>
                  <label className="block font-semibold mb-2 text-gray-800 text-sm">Assign User *</label>
      ...
      {showConfirmDialog && (
        <div className="anim-fadeIn fixed inset-0 bg-black/60 flex items-center justify-center z-[2000] p-4"
          ...
          <div className="anim-slideUp bg-white rounded-2xl w-full max-w-[420px] p-8
            ...
            <div className="text-lg font-bold text-gray-800 text-center mb-2">Delete Driver?</div>
            <div className="text-sm text-gray-500 text-center mb-6 leading-relaxed">
              Are you sure you want to delete this driver? This action cannot be undone and all associated data will be permanently removed.
            </div>
